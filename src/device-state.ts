		...
		hostConfig
			...
			.catch(err =>
				res
					.status(503)
					.send(
						(err != null ? err.message : undefined) || err || 'Unknown error',
					),
			),
	);
...
export class DeviceState extends (EventEmitter as new () => DeviceStateEventEmitter) {
	...
	public async init() {
		this.config.on('change', changedConfig => {
			...
			if (changedConfig.appUpdatePollInterval != null) {
				return (this.maxPollTime = changedConfig.appUpdatePollInterval);
			}
		});
	...
	public pausingApply(fn: () => any) {
		...
		const pause = () => {
			return Bluebird.try(() => {
				let res = null;
				this.applyBlocker = new Promise(resolve => (res = resolve));
				return res;
	...
	public triggerApplyTarget({
		...
		if (this.applyInProgress) {
			if (this.scheduledApply == null || (isFromApi && this.cancelDelay)) {
				...
				if (isFromApi) {
					...
					if (typeof this.cancelDelay === 'function') {
						this.cancelDelay();
					}
				}
			} else {
		...
		new Bluebird((resolve, reject) => {
			...
			return (this.cancelDelay = reject);
		})
			.catch(() => {
				return (this.applyCancelled = true);
			})
			.then(() => {
			...
			.finally(() => {
				...
				if (this.scheduledApply != null) {
					...
					return (this.scheduledApply = null);
				}
			});
		return null;
	}
	...
	public applyIntermediateTarget(
		...
		return this.applyTarget({ intermediate: true, force, skipLock }).then(
			() => {
				return (this.intermediateTarget = null);
			},
		);
	}
}
